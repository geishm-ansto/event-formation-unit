/** Copyright (C) 2016, 2017 European Spallation Source ERIC */

#include <common/TDCTSerializer.h>
#include <common/Producer.h>
#include <cstring>
#include <test/TestBase.h>
#include "tdct_timestamps_generated.h"

//#define ARRAYLENGTH 125000
#define ARRAYLENGTH 10

struct MockProducer {
  inline void produce(nonstd::span<const uint8_t>, int64_t)
  {
    NumberOfCalls++;
  }


  size_t NumberOfCalls {0};
};

class TDCTSerializerTest : public TestBase {
  void SetUp() override {
    for (int i = 0; i < 200000; i++) {
      time[i] = i;
    }
  }

  void TearDown() override {}

protected:
  char flatbuffer[1024 * 1024];
  uint32_t time[200000];
  TDCTSerializer fb{ARRAYLENGTH, "nameless"};
};

TEST_F(TDCTSerializerTest, Serialize) {
  for (size_t i=0; i < ARRAYLENGTH; i++)
    fb.addEvent(i);
  auto buffer = fb.serialize();
  EXPECT_GE(buffer.size_bytes(), ARRAYLENGTH * 8);
  EXPECT_LE(buffer.size_bytes(), ARRAYLENGTH * 8 + 2048);
  ASSERT_TRUE(not buffer.empty());

  EXPECT_EQ(std::string(reinterpret_cast<const char*>(&buffer[4]), 4), "tdct");
}

TEST_F(TDCTSerializerTest, SerDeserialize) {
  for (size_t i=0; i < ARRAYLENGTH-1; i++)
    fb.addEvent(i);
  auto buffer = fb.serialize();

  memset(flatbuffer, 0, sizeof(flatbuffer));

  auto events = Gettimestamp(flatbuffer);

  ASSERT_NE(events->sequence_counter(), 1);

  memcpy(flatbuffer, buffer.data(), buffer.size_bytes());
  EXPECT_EQ(std::string(&flatbuffer[4], 4), "tdct");
  events = Gettimestamp(flatbuffer);
  EXPECT_EQ(events->name()->str(), "nameless");
  ASSERT_EQ(events->sequence_counter(), 1);
}

TEST_F(TDCTSerializerTest, DeserializeCheckData) {
  for (int i = 0; i < ARRAYLENGTH - 1; i++) {
    auto len = fb.addEvent(time[i]);
    ASSERT_EQ(len, 0);
    ASSERT_EQ(fb.eventCount(), i+1);
  }

  auto buffer = fb.serialize();
  ASSERT_TRUE(not buffer.empty());

  memcpy(flatbuffer, buffer.data(), buffer.size_bytes());
  EXPECT_EQ(std::string(&flatbuffer[4], 4), "tdct");

  auto veri = flatbuffers::Verifier((uint8_t *)flatbuffer, buffer.size_bytes());
  ASSERT_TRUE(VerifytimestampBuffer(veri));
  auto events = Gettimestamp(flatbuffer);
  EXPECT_EQ(events->name()->str(), "nameless");

  auto timevec = events->timestamps();
  EXPECT_EQ(timevec->size(), ARRAYLENGTH - 1);

  for (int i = 0; i < ARRAYLENGTH - 1; i++) {
    EXPECT_EQ((*timevec)[i], i);
  }

  EXPECT_EQ(events->name()->str(), "nameless");
}

TEST_F(TDCTSerializerTest, AutoDeserialize) {
  MockProducer mp;
  auto Produce = [&mp](auto A, auto B) {
    mp.produce(A, B);
  };
  fb.setProducerCallback(Produce);

  for (int i = 0; i < ARRAYLENGTH - 1; i++) {
    auto len = fb.addEvent(time[i]);
    ASSERT_EQ(len, 0);
    ASSERT_EQ(fb.eventCount(), i + 1);
  }
  EXPECT_EQ(mp.NumberOfCalls, 0);

  auto len = fb.addEvent(time[ARRAYLENGTH - 1]);
  EXPECT_GT(len, 0);
  EXPECT_EQ(mp.NumberOfCalls, 1);
}

int main(int argc, char **argv) {
  testing::InitGoogleTest(&argc, argv);
  return RUN_ALL_TESTS();
}
